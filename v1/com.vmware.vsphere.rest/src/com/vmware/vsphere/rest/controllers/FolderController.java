...
import com.vmware.vsphere.rest.models.RESTFolder;
...
public class FolderController {
	...
	public List<RESTFolder> getEntity(@Context HttpHeaders headers, 
		...
		try {
			...
			List<RESTFolder> moList = new ArrayList<RESTFolder>();
			ManagedEntity[] e = new ViConnection().getEntities("Folder", headers, viServer);
			...
			for (ManagedEntity m : e) {
				...
				if (position >= start) {
					...
					RESTFolder mo = new RESTFolder((Folder) m, thisUri, fieldStr);
					if (sp.Match(mo)) {
						moList.add(mo);
						this.count++;
	...
	@PropertyFiltering(using = "fields", defaults = defaults)
	@Produces(MediaType.APPLICATION_JSON)
	public RESTFolder getEntityById(@Context HttpHeaders headers,
			@PathParam("viServer") String viServer,
		...
		try {
			...
			ManagedEntity m = new ViConnection().getEntity("Folder", id, headers, viServer);
