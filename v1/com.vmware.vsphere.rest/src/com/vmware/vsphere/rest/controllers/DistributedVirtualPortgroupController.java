...
import com.vmware.vsphere.rest.models.RESTDistributedVirtualPortgroup;
...
@Path("/{viServer}/distributedvirtualportgroups")
public class DistributedVirtualPortgroupController {
	...
	public List<RESTDistributedVirtualPortgroup> getEntity(@Context HttpHeaders headers, 
		...
		try {
			...
			List<RESTDistributedVirtualPortgroup> moList = new ArrayList<RESTDistributedVirtualPortgroup>();
			ManagedEntity[] e = new ViConnection().getEntities("DistributedVirtualPortgroup", headers, viServer);
			...
			for (ManagedEntity m : e) {
				...
				if (position >= start) {
					...
					RESTDistributedVirtualPortgroup mo = new RESTDistributedVirtualPortgroup((DistributedVirtualPortgroup) m, thisUri, fieldStr);
					if (sp.Match(mo)) {
						moList.add(mo);
						this.count++;
	...
	@PropertyFiltering(using = "fields", defaults = defaults)
	@Produces(MediaType.APPLICATION_JSON)
	public RESTDistributedVirtualPortgroup getEntityById(@Context HttpHeaders headers,
			@PathParam("viServer") String viServer,
		...
		try {
			...
			ManagedEntity m = new ViConnection().getEntity("DistributedVirtualPortgroup", id, headers, viServer);
