...
import com.vmware.vsphere.rest.models.RESTHostSystem;
...
public class HostSystemController {
	...
	public List<RESTHostSystem> getEntity(@Context HttpHeaders headers, 
		...
		try {
			...
			List<RESTHostSystem> moList = new ArrayList<RESTHostSystem>();
			ManagedEntity[] e = new ViConnection().getEntities("HostSystem", headers, viServer);
			...
			for (ManagedEntity m : e) {
				...
				if (position >= start) {
					...
					RESTHostSystem mo = new RESTHostSystem((HostSystem) m, thisUri, fieldStr);
					if (sp.Match(mo)) {
						moList.add(mo);
						this.count++;
	...
	@PropertyFiltering(using = "fields", defaults = defaults)
	@Produces(MediaType.APPLICATION_JSON)
	public RESTHostSystem getEntityById(@Context HttpHeaders headers,
			@PathParam("viServer") String viServer,
		...
		try {
			...
			ManagedEntity m = new ViConnection().getEntity("HostSystem", id, headers, viServer);
